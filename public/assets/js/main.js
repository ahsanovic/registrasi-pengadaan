const initAppToggler = () => {
	const syncDesktopTogglerState = () => {
		if (window.innerWidth < 1191) return;
		const currentValue = document.documentElement.getAttribute("data-app-sidebar") || "full";
		const isExpanded = !currentValue.startsWith("mini");
		document.querySelectorAll(".app-toggler").forEach((button) => {
			button.setAttribute("aria-expanded", String(isExpanded));
		});
	};

	syncDesktopTogglerState();

	if (!window.__appTogglerClickBound) {
		window.__appTogglerClickBound = true;
		document.addEventListener("click", (event) => {
			const toggler = event.target.closest(".app-toggler");
			if (!toggler) return;
			...
			if (window.innerWidth >= 1191) {
				const currentValue = document.documentElement.getAttribute("data-app-sidebar") || "full";
				const isMiniState = currentValue.startsWith("mini");
				const nextSidebar = isMiniState ? "full" : "mini";

				// Keep sidebar state persistent across Livewire navigation.
				if (typeof window.setAppSettings === "function") {
					window.setAppSettings({ appSidebar: nextSidebar });
				} else {
					document.documentElement.setAttribute("data-app-sidebar", nextSidebar);
				}
				...
				if (appMenubar) {
					...
					toggler.setAttribute("aria-expanded", String(appMenubar.classList.contains("open")));
				}
			}
		});
	}
	...
	const appMenubar = document.getElementById("appMenubar");
	if (!appMenubar || appMenubar.dataset.hoverBound) return;

	appMenubar.dataset.hoverBound = "true";
...
function initModals() {
	document.querySelectorAll('[data-bs-toggle="modal"]').forEach((modalToggle) => {
...
document.addEventListener("livewire:navigated", () => {
	// Re-init twice to handle DOM morph timing differences after SPA navigation.
	initAppToggler();
	initBootstrapComponents();
	setTimeout(() => {
		initAppToggler();
...
document.addEventListener("livewire:initialized", () => {
	// Re-bind Bootstrap behaviors after regular Livewire DOM morphs
	// (e.g. click actions that re-render table rows without navigation).
